...
typedef struct MOVContext {
    const AVClass *c; ///< class for private options
    AVFormatContext *fc;
    int time_scale;
...
static AVIndexEntry* read_stts(MOVContext *c, AVIOContext *pb) {
    MOVStreamContext *sc;
    AVStream *st;
    unsigned int i=0;
    st = c->fc->streams[0];
    sc = (MOVStreamContext *)st->priv_data;
    MOVStts d[] = {{.count=0, .duration=0}, {.count=1, .duration=1}};
    av_log(c->fc, AV_LOG_WARNING, "Num of data: %u\n", sc->stts_count);
    av_log(c->fc, AV_LOG_WARNING, "Tot count: %u\n", st->nb_frames);
    av_log(c->fc, AV_LOG_WARNING, "Tot dur: %u\n", st->duration);
    av_log(c->fc, AV_LOG_WARNING, "unsigned int size: %u\n", sizeof(sc->stts_data));
    unsigned int val = d[0].count;
    int num_entries = st->nb_index_entries;
    av_log(c->fc, AV_LOG_WARNING, "Tot entries: %u\n", st->nb_index_entries);
    AVIndexEntry *entries = (AVIndexEntry *)malloc(num_entries * sizeof(AVIndexEntry));
    for (i = 0; i < num_entries; i++) {
        AVIndexEntry *idx_entry = &st->index_entries[i];
        entries[i] = st->index_entries[i];
        av_log(c->fc, AV_LOG_WARNING, "sample count:%d\n", i);
        av_log(c->fc, AV_LOG_WARNING, "timestamp:%d\n", idx_entry->timestamp);
        av_log(c->fc, AV_LOG_WARNING, "flags:%d\n", idx_entry->flags);
        //av_log(c->fc, AV_LOG_WARNING, "Size of STTS data:%d\n",sizeof(sc->stts_data));
        //MOVStts stts_data = sc->stts_data[i];
        //av_log(c->fc, AV_LOG_WARNING, "sample count:%d\n", stts_data.count);
        //av_log(c->fc, AV_LOG_WARNING, "sample count: %d\n", stts_data.duration);
    }
    av_log(c->fc, AV_LOG_WARNING, "entry size:%d\n", sizeof(entries));
    return entries;

}

static int mov_read_stts(MOVContext *c, AVIOContext *pb)
{
    AVStream *st;
    MOVStreamContext *sc;
    ...
    if (c->fc->nb_streams < 1){
        av_log(c->fc, AV_LOG_WARNING, "Lack of streams\n");
        return 0;
    }
    ...
    av_log(c->fc, AV_LOG_WARNING, "track[%u].stts.entries = %u\n",
    ...
    if (entries >= INT_MAX / sizeof(*sc->stts_data)){
        av_log(c->fc, AV_LOG_ERROR, "entries greater tan int max\n");
    ...
    for (i = 0; i < entries && !pb->eof_reached; i++) {
        ...
        MOVStts *stts_data = (MOVStts *)av_fast_realloc(sc->stts_data, &alloc_size,
